...
const GameResults: React.FC<GameResultsProps> = ({ results, onPlayAgain }) => {
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-4 max-w-md mx-auto">
      <h1 className="text-3xl font-bold text-foreground">🏁 Resultat</h1>
      ...
      <Card className="w-full">
        <CardHeader className="text-center pb-2">
          ...
          <p className="text-muted-foreground">av {maxPossible} möjliga poäng</p>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="flex justify-between p-3 rounded-md bg-accent/50">
            <span className="text-sm font-medium">🎯 Bästa gissning</span>
            <span className="text-sm font-bold">{best.cityName} ({Math.round(best.distanceKm)} km)</span>
          </div>
          <div className="flex justify-between p-3 rounded-md bg-accent/50">
            <span className="text-sm font-medium">😅 Sämsta gissning</span>
            ...
            {results.map((r, i) => (
              <div key={i} className="flex justify-between items-center text-sm py-1 border-b border-border last:border-0">
                ...
                <div className="flex gap-3 text-muted-foreground">
                  ...
                  <span className="font-bold text-foreground w-16 text-right">{r.score} p</span>
                </div>
              </div>
            ))}
          </div>
