...
describe("Generate Uganda lakes TS multiline", () => {
  it("should output TS content with short lines", () => {
    ...
    // Build TS file with short lines (10 coords per line)
    const lines: string[] = [];
    lines.push("// Uganda lakes - high-resolution data from OpenStreetMap and Natural Earth");
    lines.push("// Auto-generated inline data - do not edit manually");
    lines.push("");
    lines.push("export interface UgandaLakeData {");
    lines.push("  name: string;");
    lines.push("  coordinates: [number, number][];");
    lines.push("}");
    lines.push("");
    lines.push("export const UGANDA_LAKES: UgandaLakeData[] = [");
    
    for (const lake of lakes) {
      lines.push(`  { name: "${lake.name}", coordinates: [`);
      // Write 10 coords per line
      for (let i = 0; i < lake.coordinates.length; i += 10) {
        const chunk = lake.coordinates.slice(i, i + 10);
        const str = chunk.map(c => `[${c[0]},${c[1]}]`).join(",");
        const comma = (i + 10 < lake.coordinates.length) ? "," : "";
        lines.push(`    ${str}${comma}`);
      }
      lines.push("  ] },");
    }
    lines.push("];");
    ...
    const outPath = path.resolve(__dirname, "../../tmp/uganda-lakes-ml.ts");
